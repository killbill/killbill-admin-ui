module Kaui
  class TagDefinitionsController < Kaui::EngineController
    ...
    def create
      ...
      respond_to do |format|
        if @tag_definition.save(current_user.kb_username, params[:reason], params[:comment], options_for_klient)
    ...
    def destroy
      @tag_definition = TagDefinition.find(params[:id], options_for_klient)
      @tag_definition.destroy(current_user.kb_username, params[:reason], params[:comment], options_for_klient)
