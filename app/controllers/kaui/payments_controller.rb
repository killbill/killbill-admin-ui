class Kaui::PaymentsController < Kaui::EngineController
  ...
  def show
    ...
    fetch_payment_fields = lambda { @custom_fields = @payment.custom_fields('NONE', options_for_klient).sort { |cf_a, cf_b| cf_a.name.downcase <=> cf_b.name.downcase } }
    ...
    run_in_parallel fetch_payment_fields, fetch_payment_method
  end
