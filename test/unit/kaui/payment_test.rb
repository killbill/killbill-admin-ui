...
class Kaui::PaymentTest < ActiveSupport::TestCase

  test 'can convert to money' do
    payment = Kaui::Payment.new(:auth_amount      => 12.42,
  ...
  test 'can check for full refunds' do
    assert Kaui::Payment.new(:purchased_amount => 10.2, :refunded_amount => 10.20, :currency => 'USD').is_fully_refunded?
    assert !Kaui::Payment.new(:purchased_amount => 10.2, :refunded_amount => 9, :currency => 'USD').is_fully_refunded?

    assert Kaui::Payment.new(:captured_amount => 10.2, :refunded_amount => 10.20, :currency => 'USD').is_fully_refunded?
    assert !Kaui::Payment.new(:captured_amount => 10.2, :refunded_amount => 9, :currency => 'USD').is_fully_refunded?
  end
end
