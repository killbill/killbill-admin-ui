...
module Kaui
  class InvoicePaymentTest < ActiveSupport::TestCase
    ...
    test 'can check for full refunds' do
      ...
      assert_not Kaui::InvoicePayment.new(purchased_amount: 10.2, refunded_amount: 9, currency: 'USD').fully_refunded?

      assert Kaui::InvoicePayment.new(captured_amount: 10.2, refunded_amount: 10.20, currency: 'USD').fully_refunded?
      assert_not Kaui::InvoicePayment.new(captured_amount: 10.2, refunded_amount: 9, currency: 'USD').fully_refunded?
    end
  end
end
